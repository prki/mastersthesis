...
\begin{figure}[h]
  ...
  \caption{A zoomed in image used in the figure \ref{fig:jfifvspng-sample}. While
           ...
           gray background easily once zoomed out. Color levels were adjusted
...
which works the best will depend to a large extent on the redundancies inherent
to the data compressed. \cite{datacompression}
\\

In the phase of modeling, information about any redundancy is extracted in a form
of a model, which can then be utilized in the compression algorithm.
\\

A very simple example of modeling and its importance can be seen on \emph{delta encoding}.


\section{Image compression}

\subsection{Modeling in image compression}
