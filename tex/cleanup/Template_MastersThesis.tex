...
\emph{grayscale} and \emph{Y'CbCr}.
...
the image while retaining the luminance. \emph{CITATION [https://www.mathworks.com/help/matlab/ref/rgb2gray.html]}.
\\
...
transformation from an associated RGB color space. \emph{[CITATION NEEDED]} The $Y'$
holds the \emph{luma} value (brightness of an image). The $C_{B}$ and $C_{R}$ values
are considered the \emph{chroma components}, representing the color information. 
