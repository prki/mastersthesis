...
def reshape_flat_data(buf, w, rank, h, dtype=np.float32):
    """ Function reshaping flattened data into matrices of shapes
    w x rank, rank x h.
    """
...
def output_test_results(compr_time, decompr_time, img_filepath, compressed_image, decompr_path, decompr_img,
    ...
    """ Function emitting the compression results into a CSV file."""
...
def compress_naive_nmf(np_w, np_h):
    """ Function used for compressing an image using the naive NMF compression
    scheme.
    """
...
def decompress_naive_nmf(compressed_data):
    """ Function used for decompressing an image which was compressed with the naive
    NMF compression scheme. The structure of the header is the following:
...
def naive_rgb_scheme(img_filepath, max_iter, rank, dtype, seed):
    """ Top-level function for compressing images using the naive RGB scheme."""
...
def compress_separate_nmf(rmat_w, rmat_h, gmat_w, gmat_h, bmat_w, bmat_h):
    """ Function used for compressing images using the separate RGB compression
    scheme.
    """
...
def decompress_separate_rgb(compressed_data):
    """ Function used for decompressing images which were compressed using the
    separate RGB compression scheme. The header structure is the following:
...
def separate_rgb_scheme(img_filepath, max_iter, rank, dtype, seed):
    """ Top-level function used for compressing images using the separate
    RGB scheme.
    """
...
def compress_ycbcr_nmf(y_mat, cb_w_mat, cb_h_mat, cr_w_mat, cr_h_mat):
    """Function compressing images using the YCbCr compression scheme."""
...
def ycbcr_scheme(img_filepath, max_iter, rank, dtype, seed):
    """Top-level function for compressing images using the YCbCr compression scheme."""
...
def test_ycbcr_scheme(img_filepath, max_iter, rank, dtype, seed):
    """ Test function which compresses an image using the YCbCr scheme, decompresses
    it and emits the test results into a CSV file for further analysis.
    """
...
def test_naive_rgb_scheme(img_filepath, max_iter, rank, dtype, seed):
    """ Test function which compresses an image using the naive RGB scheme, decompresses
    it and emits the test results into a CSV file for further analysis.
    """
    compr_time_begin = time.time()
...
def test_separate_rgb_scheme(img_filepath, max_iter, rank, dtype, seed):
    """ Test function which compresses an image using the separate RGB scheme, decompresses
    it and emits the test results into a CSV file for further analysis.
    """
    compr_time_begin = time.time()
...
def main():
    ...
    compr_img = naive_rgb_scheme(img_filepath, 300, 100, dtype=np.float32, seed='random')
    decompr_img = decompress_naive_nmf(compr_img)
    decompr_img.show()

    #filenames = ['artificial.png', 'bridge.png', 'deer.png', 'hdr.png', 'nightshot_iso_1600.png', 'spider_web.png']
    #filenames = ['nightshot_iso_1600.png']
    #for filename in filenames:
        #for max_iter in range(150, 450):
            #test_ycbcr_scheme(filename, max_iter, 150, dtype=np.float32, seed='nndsvd') 
            #test_naive_rgb_scheme(filename, max_iter, 150, dtype=np.float32, seed='random_vcol')
        #for rank in range(5, 151):
            #test_naive_rgb_scheme(filename, 300, rank, dtype=np.float32, seed='random')
